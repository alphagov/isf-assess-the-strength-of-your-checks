...
module.exports.validateResponse = function(userRiskLevel, evidence, verificationScore, fraudScore, activityScore) {
  ...
  for (var numRequiredEvidence=1; numRequiredEvidence<=filteredEvidence.length; numRequiredEvidence++) {
    ...
    for (var j=0; j<requiredEvidenceCountProfiles.length; j++) {
      ...
      if (
          scoreAdequate(matchingEvidenceCount, numRequiredEvidence) &&
          scoreAdequate(verificationScore, currentProfile.verification) &&
          scoreAdequate(fraudScore, currentProfile.fraud) &&
          scoreAdequate(activityScore, currentProfile.activity)
        ) {
          validated = true
        }
    }
